...
\begin{abstract}
We study the nonexistence of nontrivial positive cycles in the Collatz dynamics. For cycle lengths $k = 3$ to~$15$, we prove $N_0(d) = 0$ (no gap composition achieves corrsum $\equiv 0 \pmod{d}$) by exhaustive computation in Lean~4 (280 theorems, 0~\texttt{sorry}, 0~axiom). For $k \geq 18$, we prove nonsurjectivity: $\binom{S-1}{k-1} < d(k)$. Combined with Hercher~\cite{Hercher25} ($k \leq 91$), our results cover all $k \leq 91$. The gap between nonsurjectivity and $N_0(d) = 0$ for $k > 91$ remains open.
...
\begin{theorem}[Certified zero-exclusion]\label{thm:main}
For every $k$ with $3 \leq k \leq 15$ and $S = S_{\min}(k)$, the number $N_0(d)$ of compositions~$\sigma$ of~$S$ into $k$ parts $\geq 1$ with $d \mid \corr(\sigma)$ equals zero. Hence no nontrivial cycle of length $k \leq 15$ exists.
\end{theorem}
...
\corr(\sigma) = \sum_{i=0}^{k-1} 3^{k-1-i} \cdot 2^{P_i},
\end{equation}
where $P_0 = 0$ and $P_i = g_1 + \cdots + g_i$ for $i \geq 1$ are the cumulative position sums, satisfying $0 = P_0 < P_1 < \cdots < P_{k-1} < S$.
