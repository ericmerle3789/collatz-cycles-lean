...
The proof combines certified computation (Lean~4, \texttt{native\_decide}) for $k \leq 10000$ with the Baker--W\"ustholz theorem for $k \geq 10001$.
...
For $k = 2$: $d = 7$, and the composition $(2,2)$ gives $\corr = 14 = 2 \cdot 7$, hence $n_1 = 2$. But $n_1 = 2$ is even, contradicting the requirement that cycle elements are odd. No nontrivial odd cycle exists for $k = 2$.
\end{remark}

%% ============================================================
\section{The Baker--W\"ustholz Bound for $k \geq 10001$}
%% ============================================================

\subsection{Linear Forms in Logarithms}

\begin{theorem}[Baker--W\"ustholz {\cite{BW93}}, Theorem~1]\label{thm:BW}
Let $\alpha_1, \alpha_2$ be algebraic numbers with $|\alpha_i| \geq 1$, and let $b_1, b_2$ be positive integers. Define $\Lambda = b_1 \log \alpha_1 - b_2 \log \alpha_2$. If $\Lambda \neq 0$, then
\[
\log|\Lambda| > -18(d+1)!\, d^{d+1}\, (32ed)^{d+2}\, \log A_1 \cdot \log A_2 \cdot \log(eB),
\]
where $d = [\mathbb{Q}(\alpha_1, \alpha_2) : \mathbb{Q}]$, the $A_i \geq e$ satisfy $\log A_i \geq h(\alpha_i)$, and $B = \max(|b_1|/\log A_2,\, |b_2|/\log A_1)$.
\end{theorem}

\subsection{Application to Range Exclusion}

Setting $\alpha_1 = 2$, $\alpha_2 = 3$, $b_1 = S$, $b_2 = k$, $d = 1$, and $\Lambda = S\log 2 - k\log 3 > 0$, we obtain
\begin{equation}\label{eq:baker-bound}
d(k) = 2^S - 3^k > 3^k \cdot \exp(-C_0 \cdot \log S \cdot \log k),
\end{equation}
where $C_0 = 18 \cdot 2 \cdot (32e)^3 \cdot \log 3 \approx 2.6 \times 10^7$.
...
\begin{proof}
The corrsum range satisfies $\mathrm{cs_{max}} - \mathrm{cs_{min}} < 2^S$. By~\eqref{eq:baker-bound}, $d > 3^k \cdot k^{-C}$ for an effective constant~$C$, while $\mathrm{cs_{max}} < 2^S = 3^k \cdot 2^{S-k\log_2 3}$. Since $S - k\log_2 3 < 1$, we have $\mathrm{cs_{max}} < 2 \cdot 3^k$, and $d / \mathrm{cs_{max}} \to \infty$.
\end{proof}

\begin{remark}
The Lean formalization uses the threshold $k \geq 10001$ for conservatism: $k \leq 10000$ is covered by \texttt{native\_decide}, and the Baker--W\"ustholz bound is invoked as an axiom beyond this range.
\end{remark}
